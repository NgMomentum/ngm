...
var EMAIL_ADDRESS = 'myself@angular.dev';

describe('unauthenticated sample app', function() {
    beforeEach(function() {
    ...
    it('asks for authentication', function() {
        cy.visit('http://localhost:4000/#/home').get('button.btn').contains('Preferences').click();
    ...
    it('can authenticate', function() {
        ...
        cy.contains('Reset All Data').then(function() {
            var appConfig = sessionStorage.getItem('appConfig');
...
describe('authenticated sample app', function() {
    var _appConfig = null;
    beforeEach(function() {
        var applyAppConfig = function applyAppConfig() {
            window.sessionStorage.clear();
        ...
        if (!_appConfig) {
            ...
            cy.get('select').contains('myself').parent('select').select(EMAIL_ADDRESS);
            cy.get('button').contains('Log in').click();
            cy.url().should('include', '#/home').then(function() {
                var appConfig = sessionStorage.getItem('appConfig');
                expect(appConfig).not.to.equal(null);
                expect(appConfig.emailAddress).not.equal(EMAIL_ADDRESS);
    ...
    it('navigates to Preferences by url', function() {
        cy.visit('http://localhost:4000/#/prefs');
        cy.contains('Reset All Data');
    });

    it('navigates to Contacts by url', function() {
        cy.visit('http://localhost:4000/#/contacts');
        cy.contains('Select a contact');
    });

    it('navigates to Messages by url', function() {
    ...
    it('prompts to save a message being composed', function() {
        cy.visit('http://localhost:4000/#/mymessages');
        cy.url().should('include', '#/mymessages/inbox');
        cy.contains('New Message').click();
        cy.get('input#to').type('somebody@somewhere.com');
    ...
    it('navigates through folders', function() {
        cy.visit('http://localhost:4000/#/mymessages');
        cy.url().should('include', '#/mymessages/inbox');
    ...
    it('navigates through messages', function() {
        var selectMessage = function selectMessage(subject, guid) {
    ...
    it('navigates through contacts', function() {
        cy.visit('http://localhost:4000/#/contacts');

        var selectContact = function selectContact(name, id) {
