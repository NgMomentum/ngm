...
msos.provide("msos.google.prettify");
...
(function (win) {
    ...
    function combinePrefixPatterns(regexs) {
        ...
        for (i = 0, n = regexs.length; i < n; ++i) {
            var regex_i = regexs[i];
            if (regex_i.global || regex_i.multiline) {
                throw new Error('' + regex_i);
            }
            rewritten.push(
                '(?:' + allowAnywhereFoldCaseAndRenumberGroups(regex_i) + ')');
        }
    ...
    function numberLines(node, startLineNum, isPreformatted) {
        ...
        for (i = 0; // Number of lines that have been split so far.
    ...
    function $prettyPrint(opt_whenDone, opt_root) {
        ...
        function doWork() {
            var endTime = (msos.google.prettify.PR_SHOULD_USE_CONTINUATION ?
                clock.now() + 250 /* ms */ :
                Infinity);
    ...
    msos.google.prettify = {
		'version': new msos.set_version(18, 2, 4),
