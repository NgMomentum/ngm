...
msos.provide("ng.ui.router.extras.dsr");

ng.ui.router.extras.dsr.version = new msos.set_version(18, 1, 30);
...
(function(angular, undefined) {
        ...
        $provide.decorator("$state", ['$delegate', '$q', function($state, $q) {
            ...
            $state.transitionTo = function(to, toParams, options) {
                ...
                return $state_transitionTo.apply($state, arguments).then(
                    ...
                    function(err) {
                        resetIgnoreDsr();
                        return $q.reject($q.defer('ng_ui_router_extra_dsr_reject_err'), err);
                    }
                );
            };
            return $state;
    ...
    angular.module(
		...
		"$deepStateRedirect",
		['$rootScope', '$state', '$injector', function($rootScope, $state, $injector) {
        var lastSubstate = {};
    ...
    angular.module(
		...
		['$deepStateRedirect', function($deepStateRedirect) {
        // Make sure $deepStateRedirect is instantiated
		var dsr = $deepStateRedirect;

		if (msos.config.verbose) {
			msos.console.debug('ng.ui.router.extras.dsr -> $deepStateRedirect:', dsr);
		}
    }]);
