...
msos.provide("ng.material.core.gestures");
...
_.extend(
    ng.material.core.gestures,
    {
        ...
        isInitialized: false,
		maxClickDistance: 6,
        version: new msos.set_version(18, 7, 8)
    }
);
...
function MdGesture($$MdGestureHandler, $timeout) {
	"use strict";
	...
	function register(element, handlerName, options) {
		var handler = ng.material.core.gestures.HANDLERS[handlerName.replace(/^\$md./, '')];
		if (!handler) {
			throw new Error('Failed to register element with handler ' + handlerName + '. ' +
				'Available handlers: ' + Object.keys(ng.material.core.gestures.HANDLERS).join(', '));
		}
	...
	function addHandler(name, definition) {
		...
		ng.material.core.gestures.HANDLERS[name] = handler;
	...
	self = {
		...
		isIos: isIos,
		isAndroid: isAndroid,
		isHijackingClicks: (isIos || isAndroid) && !hasJQuery && !ng.material.core.gestures.forceSkipClickHijack
	};
	...
	if (self.isHijackingClicks) {
		...
		self.handler(
			'click',
			{
				options: { maxDistance: ng.material.core.gestures.maxClickDistance },
		...
		self.handler(
			'focus', {
				...
				onEnd: function (ev, oe_pointer) {
					if (oe_pointer.distance < this.state.options.maxDistance && canFocus(ev.target)) {
	...
	self.handler(
		...
		'swipe',
		{
			...
			onEnd: function (ev, oes_pointer) {
				...
				if (Math.abs(oes_pointer.velocityX) > this.state.options.minVelocity && Math.abs(oes_pointer.distanceX) > this.state.options.minDistance) {
					eventType = oes_pointer.directionX === 'left' ? '$md.swipeleft' : '$md.swiperight';
					this.dispatchEvent(ev, eventType);
				} else if (Math.abs(oes_pointer.velocityY) > this.state.options.minVelocity && Math.abs(oes_pointer.distanceY) > this.state.options.minDistance) {
...
MdGestureProvider.prototype = {
	...
	disableAll: function () {
		ng.material.core.gestures.disableAllGestures = true;
	},
    skipClickHijack: function () {
        "use strict";

        ng.material.core.gestures.forceSkipClickHijack = true;

        return ng.material.core.gestures.forceSkipClickHijack;
    },
	setMaxClickDistance: function (clickDistance) {
		ng.material.core.gestures.maxClickDistance = parseInt(clickDistance);
	},
	$get: ["$$MdGestureHandler", "$timeout", function ($$MdGestureHandler, $timeout) {
		return new MdGesture($$MdGestureHandler, $timeout);
	}]
};
...
function MdGestureHandler() {
    ...
    function jQueryDispatchEvent(srcEvent, eventType, eventPointer) {
        ...
        eventPointer = eventPointer || ng.material.core.gestures.pointer;
...
function attachToDocument($mdGesture, $$MdGestureHandler) {
	...
	if (ng.material.core.gestures.disableAllGestures) {
		return;
	}
    ...
    function isInputEventFromLabelClick(event) {
        var click_pos = ng.material.core.gestures.lastLabelClickPos;
    ...
    function clickHijacker(ev) {
        var isKeyClick = ev.clientX === 0 && ev.clientY === 0,
			isSubmitEvent = ev.target && ev.target.type === 'submit';

		if (!isKeyClick && !ev.$material && !ev.isIonicTap && !isInputEventFromLabelClick(ev) && !isSubmitEvent) {
            ev.preventDefault();
            ev.stopPropagation();
            ng.material.core.gestures.lastLabelClickPos = null;
        } else {
            ng.material.core.gestures.lastLabelClickPos = null;
            if (ev.target.tagName.toLowerCase() === 'label') {
                ng.material.core.gestures.lastLabelClickPos = { x: ev.x, y: ev.y };
            }
        }
    }

    if (!ng.material.core.gestures.isInitialized && $mdGesture.isHijackingClicks ) {
        document.addEventListener('click',      clickHijacker,      true);
        document.addEventListener('mouseup',    mouseInputHijacker, true);
        document.addEventListener('mousedown',  mouseInputHijacker, true);
        document.addEventListener('focus',      mouseInputHijacker, true);

        ng.material.core.gestures.isInitialized = true;
    }
    ...
    function runHandlers(handlerEvent, event) {
        ...
        for (name in ng.material.core.gestures.HANDLERS) {    // Leave as is...
            handler = ng.material.core.gestures.HANDLERS[name];
            if (handler instanceof $$MdGestureHandler) {
                ...
                handler[handlerEvent](event, ng.material.core.gestures.pointer);
            }
        }
    }
    ...
    function gestureStart(ev) {
        ...
        if (ng.material.core.gestures.pointer) { return; }

        now = +Date.now();

        // iOS & old android bug: after a touch event, a click event is sent 350 ms later.
        // If <400ms have passed, don't allow an event of a different type than the previous event
        if (ng.material.core.gestures.lastPointer && !typesMatch(ev, ng.material.core.gestures.lastPointer) && (now - ng.material.core.gestures.lastPointer.endTime < 1500)) {
            return;
        }

        ng.material.core.gestures.pointer = makeStartPointer(ev);

        runHandlers('start', ev);
    }

    function gestureMove(ev) {
        if (!ng.material.core.gestures.pointer || !typesMatch(ev, ng.material.core.gestures.pointer)) { return; }

        updatePointerState(ev, ng.material.core.gestures.pointer);
        runHandlers('move', ev);
    }
    ...
    function gestureEnd(ev) {
        ...
        updatePointerState(ev, ng.material.core.gestures.pointer);
        ng.material.core.gestures.pointer.endTime = +Date.now();

        if (ev.type !== 'pointercancel') {
			runHandlers('end', ev);
		}

        ng.material.core.gestures.lastPointer = ng.material.core.gestures.pointer;
        ng.material.core.gestures.pointer = null;
    }
    ...
    angular.element(
        ...
        function gestureClearCache() {
            ng.material.core.gestures.lastPointer = null;
            ng.material.core.gestures.pointer = null;
        }
    );
}
