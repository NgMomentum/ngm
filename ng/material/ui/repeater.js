...
msos.provide("ng.material.ui.repeater");

ng.material.ui.repeater.version = new msos.set_version(18, 4, 16);

ng.material.ui.repeater.NUM_EXTRA = 3;

// Load AngularJS-Material module specific CSS
ng.material.ui.repeater.css = new msos.loader();
ng.material.ui.repeater.css.load(msos.resource_url('ng', 'material/css/ui/repeater.css'));
    ...
    $$rAF(
        angular.bind(
            ...
            function () {
                ...
                $scope.$on(
                    '$destroy',
                    function ng_md_ui_repeater_cont_ctrl_on() {
...
VirtualRepeatContainerController.prototype.handleScroll_ = function () {
    ...
    offset = this.isHorizontal() ? (ltr ? this.scroller.scrollLeft : this.maxSize - this.scroller.scrollLeft) : this.scroller.scrollTop;
    ...
    numItems = Math.max(0, Math.floor(offset / itemSize) - ng.material.ui.repeater.NUM_EXTRA);

    transform = (this.isHorizontal() ? 'translateX(' : 'translateY(') + (!this.isHorizontal() || ltr ? (numItems * itemSize) : -(numItems * itemSize)) + 'px)';
...
function VirtualRepeatController($scope, $element, $attrs, $document, $rootScope, $mdUtil) {
    "use strict";
...
function VirtualRepeatDirective($parse) {
    ...
    return {
        controller: ['$scope', '$element', '$attrs', '$document', '$rootScope', '$mdUtil', VirtualRepeatController],
        priority: 1000,
...
VirtualRepeatController.prototype.virtualRepeatUpdate_ = function (items, oldItems) {
    ...
    for (i = this.newStartIndex; i < this.newEndIndex && (this.blocks[i] === null || this.blocks[i] === undefined); i += 1) {        // leave as is (null or undefined?)
    ...
    for (i; this.blocks[i] !== null && this.blocks[i] !== undefined; i += 1) {        // leave as is (null or undefined?)
...
VirtualRepeatController.prototype.updateIndexes_ = function () {
    ...
    this.newVisibleEnd = this.newStartIndex + containerLength + ng.material.ui.repeater.NUM_EXTRA;
    this.newEndIndex = Math.min(itemsLength, this.newVisibleEnd);
    this.newStartIndex = Math.max(0, this.newStartIndex - ng.material.ui.repeater.NUM_EXTRA);
};
...
function VirtualRepeatContainerDirective() {
    ...
    return {
        controller: ["$$rAF", "$mdUtil", "$mdConstant", "$parse", "$rootScope", "$window", "$scope", "$element", "$attrs", VirtualRepeatContainerController],
